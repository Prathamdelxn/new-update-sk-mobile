...
export default function LabourManagement({ project, selectedDate }) {
  ...
  const fetchData = async (hideLoader = false) => {
    ...
    try {
      ...
      if (!labourRes.ok) {
        const errData = await labourRes.json().catch(() => ({}));
        showToast(errData.message || 'Failed to load labourers', 'error');
        return;
      }

      let labourData = await labourRes.json();

      // Attendance is best-effort — a failure here shouldn't hide the labourer list
      let attData = [];
      if (attRes.ok) {
        attData = await attRes.json();
      } else {
        showToast('Failed to load attendance for this date', 'error');
      }
      ...
      showToast('Failed to load labour data', 'error');
    } finally {
  ...
  const handleSaveLabour = async () => {
    ...
    if (!newLabour.name || !newLabour.wageAmount) {
      showToast('Please fill name and wage amount', 'error');
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        showToast(editingLabourId ? 'Labourer updated' : 'Labourer added successfully', 'success');
        closeAddModal();
        fetchData(); // Refresh list
      } else {
        const errData = await res.json().catch(() => ({}));
        showToast(errData.message || 'Failed to save labourer', 'error');
      }
    } catch (e) {
  ...
  const handleDeleteLabour = async () => {
    ...
    try {
      ...
      if (res.ok) {
        showToast('Labourer deleted', 'success');
        closeAddModal();
        fetchData();
      } else {
  ...
  const handleDownloadPayroll = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        showToast('Failed to download payroll', 'error');
        return;
      }
...
const s = StyleSheet.create({
  ...
  searchInput: { flex: 1, backgroundColor: '#fff', paddingHorizontal: 16, paddingVertical: 10, borderRadius: 12, borderWidth: 1, borderColor: '#E0F2FE', fontFamily: 'Inter-Medium', fontSize: 14, color: '#0F172A' },
  ...
  input: { backgroundColor: '#F8FAFC', borderWidth: 1, borderColor: '#E0F2FE', borderRadius: 12, padding: 14, fontSize: 15, fontFamily: 'Inter-Medium', marginBottom: 20, color: '#0F172A' },
